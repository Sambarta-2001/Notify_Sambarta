...
const Button: React.FC<ButtonProps> = ({ children, variant = 'primary', className = '', isLoading = false, ...props }) => {
  ...
  const variantStyles = {
    ...
    secondary: 'text-gray-700 bg-gray-200 hover:bg-gray-300 focus:ring-primary-500 disabled:bg-gray-100',
    danger: 'text-white bg-red-600 hover:bg-red-700 focus:ring-red-500 disabled:bg-red-300',
  };

  const disabledClasses = isLoading ? 'cursor-not-allowed' : '';

  return (
    <button
      className={`${baseStyles} ${variantStyles[variant]} ${disabledClasses} ${className}`}
